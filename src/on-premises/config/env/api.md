...
| SWIFT_USE_TEMP_URLS                      | True                                                                                         | Generate temporary URLs for file access (allows files to be accessed without a permissive ACL).                    |
| SWIFT_TEMP_URL_KEY                       | None                                                                                         | Temporary URL key.                                                                                                 |
| SWIFT_TEMP_URL_DURATION                  | 2 * 60 * 60                                                                                  | How long a temporary URL remains valid, in seconds.                                                                |
...
| ADMIN_URL                        | ""                                  | Use to be able to deploy separate admin interface. |
| ADMIN_INTERFACE                  |                                     | Use to be able to deploy separate admin interface. |
| DJANGO_SETTINGS_MODULE           | "bimdata.settings.api"              | Use to be able to deploy separate admin interface. |
| ENV                              | "development"                       |                                                    |
| ALLOWED_HOSTS                    | []                                  |                                                    |
